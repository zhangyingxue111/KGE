...
def init_embeddings(entity_file, relation_file):
    ...
    #-------------------------------------------加入type方法1:将Bert后vec与transE后vec相加
    entity_type_emb=[]
    with open('data/FB15k-237/entity2vec_type.txt') as f:
        for line in f:
            entity_type_emb.append([float(val) for val in line.strip().split()])
    entity_new_emb=[]
    for i ,j in zip(entity_emb,entity_type_emb):
        new=[]
        for m,n in zip(i,j):
            m=m+n
            new.append(float('%3f'%m))
        entity_new_emb.append(new)
    entity_emb=entity_new_emb
    # ===================================================================================
    #-------------------------------------------加入type方法2:将Bert后vec与transE后vec相乘
    # entity_type_emb=[]
    # with open('data/FB15k-237/entity2vec_type.txt') as f:
    #     for line in f:
    #         entity_type_emb.append([float(val) for val in line.strip().split()])
