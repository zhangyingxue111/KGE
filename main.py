...
def parse_args():
    ...
    args.add_argument("-b_gat", "--batch_size_gat", type=int,
                      default=272115, help="Batch size for GAT")
...
def load_data(args):
    ...
    if args.pretrained_emb:
        # 嵌入type方法一：串联，，，，，将原来的entity2vec.txt改为entity2vec_type.txt
...
def train_gat(args):
    ...
    for epoch in range(args.epochs_gat):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            # forward pass  向前传播
            # 获得了新的entity_embed, relation_embed
        ...
        loss_gat.append(sum(epoch_loss) / len(epoch_loss))
        save_model(model_gat, args.data, epoch,
                   args.output_folder)

        
    with open("data/FB15k-237/loss_result/gat_loss_mlp(2,2).txt", "a") as f3:
        for sub_re in loss_gat:
            f3.write(str(sub_re))
            f3.write('\n')


# -----------------------------------------将entity2vec_type.txt转化为tensor
# e_type=np.loadtxt('data/FB15k-237/entity2vec_type_200.txt')
# # print(e_type)
# e_type= torch.from_numpy(e_type)
# print(e_type)
# =======================================================================


# min_emb=torch.ones(14541,100)
# temp=torch.Tensor()
# sss=''
exx=[]
# n=0
...
#  
    ...
    with open("data/FB15k-237/loss_result/convKB_loss_mlp(2,2).txt", "a") as f4:
...
# with open("data/FB15k-237/after_GAT_vec.txt", "w")as f1:
#     for i in min_emb:
#         c=1
#         for ii in i:
#             if c==1:
...
#duide 
with open("data/FB15k-237/after_GAT_vec/after_BAT_vec_mlp(2,2).txt", "w")as f1:
    for e in exx:
        c=1
        for ii in e:
            if c==1:
