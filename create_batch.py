...
class Corpus:
    def __init__(self, args, train_data, validation_data, test_data, entity2id,
        ...
        # train_data=(train_triples, train_adjacency_mat)=triples_data, (rows, cols, data)
        # triples_data=(entity2id[e1], relation2id[relation], entity2id[e2]))
        ...
        adj_values = torch.LongTensor(train_data[1][2])  # data
        self.train_adj_matrix = (adj_indices, adj_values)  # (rows, cols, data)
    ...
    def get_validation_pred(self, args, model, unique_entities):
        ...
        results=[]
        results.append("\nCumulative stats are -> ")
        results.append("Hits@100 are {}".format(cumulative_hits_100))
        results.append("Hits@10 are {}".format(cumulative_hits_ten))
        results.append("Hits@3 are {}".format(cumulative_hits_three))
        results.append("Hits@1 are {}".format(cumulative_hits_one))
        results.append("Mean rank {}".format(cumulative_mean_rank))
        results.append("Mean Reciprocal Rank {}".format(cumulative_mean_recip_rank))
        
        with open("data/FB15k-237/result.txt", "a") as f3:
